...
async function main() {
  ...
  assert.match(configPage, /if \(!syncView\.hasUnsavedWork\(\)\) loadConfig/);
  assert.match(jokesPage, /if \(!syncView\.hasUnsavedWork\(\)\) renderJokes/);
  ...
  assert.match(songBankScript, /!syncView\.hasUnsavedWork\(\)\) renderSongs/);
  assert.match(guestBankScript, /!syncView\.hasUnsavedWork\(\)\) renderGuests/);
  ...
  const syncViewScript = fs.readFileSync("js/sync-view.js", "utf8");
  ...
  assert.match(syncViewScript, /Newer saved data is available/);
  assert.match(syncViewScript, /Discard & load latest/);
  assert.match(syncViewScript, /Continue editing/);
  assert.match(syncViewScript, /Retry now/);
  ...
  ["config.html", "jokes.html", "songs.html", "guests.html", "show_management.html"].forEach(function(page) {
    assert.match(fs.readFileSync(page, "utf8"), /<script src="js\/sync-view\.js"><\/script>/);
  });
  ["top3.html", "postproduction.html"].forEach(function(page) {
    assert.doesNotMatch(fs.readFileSync(page, "utf8"), /js\/sync-view\.js/);
  });
  assert.match(showManagement, /data-edit-assignment-guidance/);
  assert.match(showManagement, /async function updateReleaseDate/);
  assert.match(showManagement, /await ShowEngine\.setReleaseDate/);
  assert.match(showManagement, /Your input is preserved/);
